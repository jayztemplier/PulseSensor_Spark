...
void pulseISR(void) {
  ...
  if(Signal2 < thresh2 && N > (IBI/5)*3){       // avoid dichrotic noise by waiting 3/5 of last IBI
  ...
  if (N > 250){                                   // avoid high frequency noise
    if ( ((Signal > thresh) || (Signal2 > thresh2)) && (Pulse == false) && (N > (IBI/5)*3) ){
      ...
      interrupts();
        ...
        for(int i=0; i<=8; i++){                // shift data in the rate array
          ...
          runningTotal += rate2[i];              // add up the 9 oldest IBI values
        }

        rate2[9] = IBI;                          // add the latest IBI to the rate array
        runningTotal += rate2[9];                // add the latest IBI to runningTotal
        runningTotal /= 10;                     // average the last 10 IBI values
        BPM2 = 60000/runningTotal;               // how many beats can fit into a minute? that's BPM!
        QS = true;                              // set Quantified Self flag
        // QS FLAG IS NOT CLEARED INSIDE THIS ISR
  ...
  if (Signal2 < thresh2 && Pulse == true){   // when the values are going down, the beat is over
    digitalWrite(blinkPin,LOW);            // turn off pin 13 LED
    Pulse = false;                         // reset the Pulse flag so we can do it again
